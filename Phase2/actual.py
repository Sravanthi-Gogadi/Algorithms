...
# List specifying the type of vehicle, zipcode, and vehicle's availability at that particular zipcode.
availability_list = [
...
# list consisting of all the nodes
all = []
allnodes = [node1, node2, node3, node4, node5, node6, node7, node8, node9]

# checking for the availability of the requested type of the vehicle
...
# Creates object for algorithm class to access the method
al = Algorithm()
...
# Match the requested node with the one in the dict.keys() and calculate the shortest path from requested nodes to others
for i in dic.keys():
    if i == zip:
        al.calculateShortestPath(allnodes, dic[i])
#Appending the distances of the available nodes in the form of a tuple
dis = []
for i in all:
    dis.append(al.getShortestPathTo(i))

# Sorting using Tim Sort - an in built sort in python
dis.sort(key=lambda tup: tup[0])
print("The available node is at a distance of {} miles from {}".format(dis[0][0], dis[0][1].name))
req = dis[0][1]

# Sending request to the nearest availability node
al.getPath(req)